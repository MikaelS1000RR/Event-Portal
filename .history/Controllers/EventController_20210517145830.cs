...
namespace Event_Portal.Controllers
{
  ...
  public class EventController : ControllerBase
  {
    ...
    public async Task<String> SelfDeletedEvents(IEnumerable<Event> events)
    {
      DateTimeOffset currentTime = DateTimeOffset.Now;
      Console.WriteLine(currentTime);
      ...
      foreach (var i in events)
      {
        if (currentTime >= i.EndDateTime)
        {
        var response = await client.DeleteTaskAsync("events/" + i.Id);
        }
        
      }

      return "Events with exceeded dates have been self deleted." + events;
